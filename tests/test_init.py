...
class TestInit:
    ...
    def test_server_main_called(self, mock_asyncio_run):
        ...
        with patch('argparse.ArgumentParser'):
            ...
            with patch.dict('sys.modules', {'src.mcp_remote_macos.server': MagicMock(main=mock_server_main)}):
                with patch.object(src.mcp_remote_macos, 'server', MagicMock(main=mock_server_main)):
                    # Act
                    src.mcp_remote_macos.main()
    ...
    def test_package_exports(self):
        ...
        # Assert
        assert "main" in src.mcp_remote_macos.__all__
        assert hasattr(src.mcp_remote_macos, "main")
        assert callable(src.mcp_remote_macos.main) 