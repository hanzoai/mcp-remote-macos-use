...
import src.mcp_remote_macos.server as server_module
from src.mcp_remote_macos.server import main
...
def mock_mcp_server():
    ...
    with patch('src.mcp_remote_macos.server.Server') as mock_server_class:
...
def mock_stdio_server():
    ...
    with patch('src.mcp_remote_macos.server.mcp.server.stdio.stdio_server') as mock_stdio:
...
async def test_main_server_initialization(mock_env_vars, mock_mcp_server, mock_stdio_server):
    ...
    server_class_mock = sys.modules['src.mcp_remote_macos.server'].Server
...
@pytest.mark.asyncio
@patch('src.mcp_remote_macos.server.handle_remote_macos_get_screen')
        ...
        async def handle_call_tool(name: str, arguments: dict = None) -> list:
            ...
            try:
                ...
                if name == "remote_macos_get_screen":
                    from src.mcp_remote_macos.server import handle_remote_macos_get_screen
    ...
    with patch.dict('os.environ', {
        ...
        with pytest.raises(ValueError, match="MACOS_HOST environment variable is required but not set"):
            ...
            with patch.dict('sys.modules'):
                if 'src.mcp_remote_macos.server' in sys.modules:
                    del sys.modules['src.mcp_remote_macos.server']
                import src.mcp_remote_macos.server
